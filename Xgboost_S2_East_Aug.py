...
# -*- coding: utf-8 -*-
"""
Created on Wed Aug 13 01:40:12 2025
...
FN=['East_Exit_Flag','Arrival_Rate5min', 'Arrival_Rate10min',
...
X_train, X_testpre, y_train, y_testpre = train_test_split(
     X, y, test_size=0.2, stratify=y,random_state=RDN
 )


X_test, X_val, y_test, y_val = train_test_split(
     X_testpre, y_testpre, test_size=0.5,stratify=y_testpre,random_state=RDN
 )
...
gsearch1 = RandomizedSearchCV(estimator = XGBClassifier(num_class=7,learning_rate =0.12, max_depth=6,
 min_child_weight=1, gamma=0, subsample=0.8, colsample_bytree=0.8,
 objective= 'multi:softmax', nthread=4, scale_pos_weight=1, seed=RDN), 
 param_distributions=param_test1,n_iter=100, scoring='accuracy',n_jobs=4, cv=5)
...
yb_test= label_binarize(y_test, classes=[0,1,2,3,4,5,6])
...
NAMES=['E1', 'E3', 'E5','C', 'D', 'E11','End-Around']
...
plt.title(" East Flow Taxi Cross ROC AUC XGBoost")
plt.legend(loc="lower right")
plt.grid(True)
#plt.show()
plt.savefig('EF_XG_CROSS_ROCAUC.jpg', dpi=800)
#plt.show()
...
plt.title("East Flow Taxi Cross Precision–Recall XGBoost")
plt.legend(loc="lower left")
plt.grid(True)
#plt.show()
plt.savefig('EF_XG_CROSS_PR.jpg', dpi=800)
plt.show()
...
disp = ConfusionMatrixDisplay(cm, display_labels=['E1', 'E3', 'E5','C', 'D','E11','End-Around'])
disp.plot(cmap=plt.cm.Blues)
plt.title("East Flow Taxi Cross Confusion Matrix XGBoost")
#plt.show()
plt.savefig('EF_XG_CROSS_CM.jpg', dpi=800)
# plt.show()
# # print()
# # print()
...
shap.summary_plot(
    ...
    class_names=['E1', 'E3', 'E5','C', 'D','E11','End-Around'],
...
plt.savefig("EF_XG_CROSS_SHAP.png", dpi=600, bbox_inches="tight", pad_inches=0.3)
plt.close(fig)
